...
### 03.30 Tabla categorías (06:35:11)
...
### 03.31 Mostrar empresa por usuario (06:44:30)

1. Creamos el archivo **`src\db\sql\functions\companyGetById.sql`**, que luego debemos correr en `Supabase`:
```js
...
COMMIT;
```
2. Abrimos el archivo **`src\supabase\crudCompanies.tsx`** y agregamos una función exportable de nombre `GetCompanyByIdUser`:
```js
...
```
3. Actualizo el `Auto Barrel` o el archivo **`index.ts`**.
4. Como debemos asegurarnos que los datos de `companies` existan (es decir la empresa), debemos abrir el archivo **`src\routes\MyRoutes.tsx`**, para insertar estas validaciones. <br/> Esto para el próximo video.
